...
void CpatchOrganizerS::writePatches2(const std::string prefix, bool bExportPLY, bool bExportPatch, bool bExportPSet) {
	...
	if (bExportPLY)
	{
		...
		if (!bOrigin)
		{
			writeFace2(m_ppatches, (prefix + "folder.ply").c_str());
		}
		else
		{
			writeFace(m_ppatches, (prefix + "folder.ply").c_str());
		}
	}
...
void CpatchOrganizerS::updateDepthMaps(Ppatch& ppatch)
{
	for (int image = 0; image < m_fm.m_tnum; ++image)
	{
		...
		for (int j = 0; j < 2; ++j)
		{
			for (int i = 0; i < 2; ++i)
			{
				if (xs[i] < 0 || m_gwidths[image] <= xs[i] || ys[j] < 0 || m_gheights[image] <= ys[j])
					continue;
...
int CpatchOrganizerS::isVisible(const Cpatch& patch, const int image,const int& ix, const int& iy,const float strict, const int lock) 
{
	...
	//若patch比dgrid内的在前，则diff为负，可视；否则，diff在后小于一定距离也可（角度垂直最不能忍）
	...
	while (bimage != eimage) {
		...
		for (int j = -margin; j <= margin; ++j) {
			...
			for (int i = -margin; i <= margin; ++i) 
			{
				...
				while (bpatch != epatch) {
					if (m_fm.isNeighborRadius(patch, **bpatch, unit,m_fm.m_neighborThreshold * scale,radius))
	...
	while (bpatch != bend) {
		...
		if (mode == 0) {
			...
			for (int i = 0; i < (int)(*bpatch)->m_images.size(); ++i) 
			{
				...
				Vec3f pos2D = m_fm.m_pss.project(image, (*bpatch)->m_coord, m_fm.m_level);
				if (pos2D[0] < 1 || pos2D[0] >= m_fm.m_pss.getWidth(image, m_fm.m_level) - 1 || pos2D[1] < 1 || pos2D[1] >= m_fm.m_pss.getHeight(image, m_fm.m_level) - 1)
				{
					continue;
				}
...
void CpatchOrganizerS::writeFace(const std::vector<Patch::Ppatch>& patches, const std::string filename)
{
	ofstream ofstr;
	ofstr.open(filename.c_str());
	ofstr << "ply" << '\n'
		<< "format ascii 1.0" << '\n'
		<< "element vertex " << (int)patches.size() * 4 << '\n'
		<< "property float x" << '\n'
		<< "property float y" << '\n'
		<< "property float z" << '\n'
		// 		<< "property float nx" << '\n'
	...
	while (bpatch != bend)
	{
		...
		//位置
		float fUnit = 0.025;
		Vec3f curNormal((*bpatch)->m_normal[0], (*bpatch)->m_normal[1], (*bpatch)->m_normal[2] );
		Vec3f xaxis, yaxis;
		xaxis = Vec3f(m_fm.m_pss.m_photos[0].m_projection[0][0][0], m_fm.m_pss.m_photos[0].m_projection[0][0][1], m_fm.m_pss.m_photos[0].m_projection[0][0][2]);
		yaxis = cross(curNormal, xaxis);
		xaxis = cross(yaxis, curNormal);
		unitize(yaxis);
		unitize(xaxis);
		yaxis *= fUnit;
		xaxis *= fUnit;
		Vec3f center((*bpatch)->m_coord[0], (*bpatch)->m_coord[1], (*bpatch)->m_coord[2]);

		list<Vec3f> lPointCood;
		lPointCood.push_back(center - xaxis - yaxis );
		lPointCood.push_back(center + xaxis - yaxis );
		lPointCood.push_back(center - xaxis + yaxis );
		lPointCood.push_back(center + xaxis + yaxis );

		for (auto it = lPointCood.begin(); it != lPointCood.end();it++)
		{
			ofstr << (*it)[0] << ' '
				<< (*it)[1] << ' '
				<< (*it)[2] << ' '
	...
	//开始输出面！！！
	for (int i = 0; i < m_ppatches.size(); i++)
	{
		int nBase = i * 4;
		ofstr << 4 << ' '
			<< nBase << ' '
...
void CpatchOrganizerS::writeFace2(const std::vector<Patch::Ppatch>& patches,const std::string filename)
{
	ofstream ofstr;
	ofstr.open(filename.c_str());
	ofstr << "ply" << '\n'
		<< "format ascii 1.0" << '\n'
		<< "element vertex " << (int)patches.size()*6 << '\n'
		<< "property float x" << '\n'
		<< "property float y" << '\n'
		<< "property float z" << '\n'
// 		<< "property float nx" << '\n'
...
// 		<< "property float nz" << '\n'
	...
	while (bpatch != bend) 
	{
		...
		//位置
		float fUnit = 0.05;
		list<Vec4f> lPointCood;
		lPointCood.push_back((*bpatch)->m_coord + (*bpatch)->m_negxaxis * fUnit - (*bpatch)->m_yaxis * fUnit * 0.5);
		lPointCood.push_back((*bpatch)->m_coord - (*bpatch)->m_yaxis * fUnit * 0.5);
		lPointCood.push_back((*bpatch)->m_coord + (*bpatch)->m_xaxis * fUnit - (*bpatch)->m_yaxis * fUnit * 0.5);
		lPointCood.push_back((*bpatch)->m_coord + (*bpatch)->m_negxaxis * fUnit + (*bpatch)->m_yaxis * fUnit * 0.5);
		lPointCood.push_back((*bpatch)->m_coord + (*bpatch)->m_yaxis * fUnit * 0.5);
		lPointCood.push_back((*bpatch)->m_coord + (*bpatch)->m_xaxis * fUnit + (*bpatch)->m_yaxis * fUnit * 0.5);
		
		int nCount = 0;
		for (list<Vec4f>::iterator it = lPointCood.begin(); it != lPointCood.end(); it++)
		{
			ofstr << (*it)[0] << ' '
				<< (*it)[1] << ' '
				<< (*it)[2] << ' ';
			switch (nCount % 3)
			{
			case 0:ofstr << 255 << ' ' << 0 << ' ' << 0 << '\n';
				break;
			case 1:ofstr << color[0] << ' ' << color[1] << ' ' << color[2] << '\n';
				break;
			case 2:ofstr << 0 << ' ' << 255 << ' ' << 0 << '\n';
				break;
			default:
				break;
			}
