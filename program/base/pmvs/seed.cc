...
void Cseed::run(void) {
	...
	// If there already exists a patch, don't use。每一个grid中counts设置为实验次数阈值（不会使用）
...
//index为乱序版图片id（内置），多线程读取index进行match初始化
		...
		for (int x = 0; x < gwidth; ++x) {
			...
			for (int p = 0; p < (int)m_ppoints[index][index2].size(); ++p) {
				...
				for (int i = 0; i < (int)vcp.size(); ++i) 
				{
					...
					m_fm.m_optim.getPAxes(index, patch.m_coord, patch.m_normal, patch.m_xaxis, patch.m_yaxis);
					unitize(patch.m_xaxis);
					unitize(patch.m_yaxis);
					patch.m_negxaxis = -patch.m_xaxis;
...
void Cseed::collectCells(const int index0, const int index1,const Cpoint& p0, std::vector<Vec2i>& cells) 
{
	...
	if (line[0] == 0.0 && line[1] == 0.0) {
		cerr << "Point right on top of the epipole? 点正好在外极点上"
			<< index0 << ' ' << index1 << endl;
		return;
	}
	// vertical，直线距离2：判断直线 若斜率 ＞１：横向， 若斜率＜１：纵向！
	...
	const Cpoint& point, std::vector<Ppoint>& vcp) 
{
	...
	for (int i = 0; i < (int)vcp.size(); ++i) {
		...
		//??? from the closest
		vcp[i]->m_response =fabs(norm(vcp[i]->m_coord - m_fm.m_pss.m_photos[index].m_center) - norm(vcp[i]->m_coord - m_fm.m_pss.m_photos[vcp[i]->m_itmp].m_center));
...
// starting with (index, indexs), set visible images by looking at correlation.处理失败返回1
int Cseed::initialMatchSub(const int index0, const int index1,
	...
	{
		m_fm.m_optim.refinePatch2(patch, id, 100);
	}
