...
void Cfilter::filterOutsideThread(void) {
	...
	for (int p = begin; p < end; ++p) {
		...
		for (int i = 0; i < vsize; ++i) 
		{
			...
			//注意，这里仍是pgrids而非vpgrids
			for (int j = 0; j < (int)m_fm.m_pos.m_pgrids[index][index2].size(); ++j) {
				const float bdepth = m_fm.m_pss.computeDepth(index, m_fm.m_pos.m_pgrids[index][index2][j]->m_coord);
				if (pdepth < bdepth &&!m_fm.isNeighbor(*ppatch, *m_fm.m_pos.m_pgrids[index][index2][j],m_fm.m_neighborThreshold1)) 
				{
					maxpressure = max(maxpressure,m_fm.m_pos.m_pgrids[index][index2][j]->m_ncc - m_fm.m_nccThreshold);
				}
			}
			m_gains[p] -= maxpressure;
		}
...
void Cfilter::filterExact(void) {
	...
	for (int p = 0; p < psize; ++p) 
	{
		...
		for (int i = 0; i < (int)m_removeimages[p].size(); ++i) 
		{
			...
			m_fm.m_pos.m_pgrids[index][index2].erase(remove(m_fm.m_pos.m_pgrids[index][index2].begin(),m_fm.m_pos.m_pgrids[index][index2].end(),m_fm.m_pos.m_ppatches[p]),m_fm.m_pos.m_pgrids[index][index2].end());
		}
	}
...
//多线程，判断每一个patch images的深度可视情况（宽松条件），不可视则push到remove，可视则new
...
void Cfilter::filterSmallGroups(void) {
	...
	//其实这里并不需要untouch，label 为 -1 即表示untouch
	while (!untouch.empty()) 
	{
		//该过程类似于种子填充
...
void Cfilter::filterWrongView(void)
{
	time_t cutTime;
	time(&cutTime);
	time_t bTime = cutTime;
	cerr << "FilterWrongView:\t" << flush;
	m_fm.m_count = 0;
	vector<thrd_t> vThreads(m_fm.m_CPU);
	for (int i = 0; i < m_fm.m_CPU; i++)
	{
		thrd_create(&vThreads[i], &filterWrongViewThreadTmp, (void*)this);
	}
	for (int i = 0; i < m_fm.m_CPU;i++)
	{
		thrd_join(&vThreads[i], NULL);
	}
// 	(*listPatch.begin()) - *(++listPatch.begin());
// 	sort(listPatch.begin(), listPatch.end());
// 	listPatch.erase(unique(listPatch.begin(), listPatch.end()), listPatch.end());
	int nCount = 0;
	for (auto it = listPatch.begin(); it != listPatch.end(); it++)
	{
		(*it)->m_flag = 0;
	}
	for (auto it = listPatch.begin(); it != listPatch.end(); it++)
	...
	time(&cutTime);
	cerr << (int)m_fm.m_pos.m_ppatches.size() << " -> "
		<< (int)m_fm.m_pos.m_ppatches.size() - nCount << " ("
		<< 100 * ((int)m_fm.m_pos.m_ppatches.size() - nCount) / (float)m_fm.m_pos.m_ppatches.size()
		<< "%)\t" << (cutTime - bTime) / CLOCKS_PER_SEC << " secs" << endl;

}

int Cfilter::filterWrongViewThreadTmp(void* arg)
{
	((Cfilter*)arg)->filterWrongViewThread();
	return 0;
}

void Cfilter::filterWrongViewThread(void)
{
	list<Ppatch> myListPatch;
	while (true)
	{
		mtx_lock(&m_fm.m_lock);
		int index = m_fm.m_count++;
		mtx_unlock(&m_fm.m_lock);
		if (index >= m_fm.m_pos.m_dpgrids.size())
		{
			mtx_lock(&m_fm.m_lock);
			listPatch.insert(listPatch.end(), myListPatch.begin(), myListPatch.end());
			mtx_unlock(&m_fm.m_lock);
			break;
		}
		
		vector<Ppatch>& grid(m_fm.m_pos.m_dpgrids[index]);
		for (int i = 0; i < grid.size();i++)
		{
			Ppatch curPatch = grid[i];
			if (curPatch == m_fm.m_pos.m_MAXDEPTH)
			{
				continue;
			}
			bool bRemove(false);
			//被看到，看看其ncc是否足够大
			vector<float> tex1(m_fm.m_wsize * m_fm.m_wsize * 3, 0);
			vector<float> tex2(m_fm.m_wsize * m_fm.m_wsize * 3, 0);
			m_fm.m_optim.getPAxes(curPatch->m_images[0], curPatch->m_coord, curPatch->m_normal, curPatch->m_xaxis, curPatch->m_yaxis);
			m_fm.m_optim.grabTex(curPatch->m_coord, curPatch->m_xaxis, curPatch->m_yaxis, curPatch->m_normal, curPatch->m_images[0], m_fm.m_wsize, tex1);
			if (m_fm.m_optim.grabTex(curPatch->m_coord, curPatch->m_xaxis, curPatch->m_yaxis, curPatch->m_normal, index, m_fm.m_wsize, tex2)
			)
			{
				bRemove = true;
			}
			...
			{
				m_fm.m_optim.normalize(tex1);
				m_fm.m_optim.normalize(tex2);
				if (m_fm.m_optim.dot(tex1, tex2) < m_fm.m_nccThresholdBefore)
				{
					bRemove = true;
				}
			}

// 			for (int k = 0; k < curPatch->m_images.size(); k++)
// 			{
// 				if (curPatch->m_images[k] == index)
// 				{
// 					bRemove = false;  
// 					break;
// 				}
// 			}
			if (bRemove)
			{
				myListPatch.push_back(curPatch);
			}
		}
