...
void Cexpand::expandThread(void) {
	...
	while (1) {
		...
		// For each direction;命名可以直接用vector<Vec4f>
...
void Cexpand::findEmptyBlocks(const Ppatch& ppatch, std::vector<std::vector<Vec4f> >& canCoords)
{
	...
	const float radiuslow = radius / 6.0f;//2.0f;（在第二近图片上，不到半个像素宽，可以判定不插值）
	const float radiushigh = radius * 2.5f;//2.0f;//1.5f;（在第二近图片上不应超过3.75个csize，太远了）
	...
	while (bpatch != epatch)
	{
		//findNeighbors 只是判断纵向像素距离（在某些视角范围内），此处判断横向距离
		const Vec4f diff = (*bpatch)->m_coord - ppatch->m_coord;
		//要求f2，何须强行划分xdir和ydir！直接用dis - nor * dis 就好啦！！！
