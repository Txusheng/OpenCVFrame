...
//   m_CPU = 1;
  ...
  m_angleThreshold1 = 75.0f * M_PI / 180.0f;
                                 ...
                                 const float radius) const
{
  ...
  const float f1 = rhs.m_normal * diff;   
  float ftmp = (fabs(f0) + fabs(f1)) / 2.0;
  //ftmp为patch距离在 patch法向量的投影差 在平均image上对应的像素（单位方便）
  ftmp /= vunit;

  // this may loosen the isneighbor testing. need to tighten (decrease) threshold?  
  //hsize 为patch距离在 patch法向量垂直向量的投影差 除以hunit
  const float hsize = norm(2 * diff - lhs.m_normal * f0 - rhs.m_normal * f1) / 2.0 / hunit;

  // radius check （ h向 * cos / dis  大于 c / f  1.5 时）
  if (radius / hunit < hsize)
    return 0;
  
  // h向 / dis 大于 c / f 时 , 及patch侧向对image时，阈值可以放宽！！！
  if (1.0 < hsize)
    ftmp /= min(2.0f, hsize);
...
//   m_CPU = 1;
  ...
  {
	  avencc += m_pos.m_ppatches[i]->m_ncc;
  }
  avencc /= m_pos.m_ppatches.size();
  //----------------------------------------------------------------------
  // Expansion
  const int TIME = 4;
